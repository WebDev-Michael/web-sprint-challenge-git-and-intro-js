...
function removeArtist(artist, index) {
  let newArtist = artist.filter((item, i) => index != i);
  return newArtist
}
...
console.log('task 6:', addArtist(artists, 'Michael', '1994 - current', 'Web Design', 'Caucasian', 'Like web development, chainsaw carving, and fish keeping. Will do anything for family and refuse to back down.'));
...
function lotsOfArt(array) {
  let newArray = array.filter(art => art.paintings >= 100).map(({name}) => name);
...
function artistByCountry(array, string){
  let newArray = array.filter(nat => nat.nationality === string).map(({name}) => name);
  return newArray;
}
